...
const statusByCode: Record<string, number> = {
  ...
  ORDER_PRICING_INVALID: 422,
  ORDER_DISCOUNT_INVALID: 422,
  ORDER_PRICING_LOCKED: 409,
  ORDER_ITEM_MISMATCH: 422,
...
function humanMessage(code: string): string {
  const messages: Record<string, string> = {
    ...
    FORBIDDEN: "This account cannot perform the requested operation.",
    ACTOR_REQUIRED: "The calling admin account is required.",
    ...
    STALE_WRITE: "This record was updated from another device. Reload and try again.",
    RESET_IN_PROGRESS: "Another admin is already resetting application data.",
    SETTINGS_INVALID: "The shop settings payload is invalid.",
    ...
    ORDER_PRICING_INVALID: "The order pricing values are invalid.",
    ORDER_DISCOUNT_INVALID: "The discount cannot exceed the order subtotal.",
    ORDER_PRICING_LOCKED:
      "Delivered or cancelled orders cannot change pricing.",
    ORDER_ITEM_MISMATCH: "The submitted order lines do not match the order.",
